...
-- Volcando datos para la tabla agencia.permiso: ~0 rows (aproximadamente)
...
-- Volcando datos para la tabla agencia.permisoherramienta: ~0 rows (aproximadamente)
...
-- Volcando datos para la tabla agencia.permisoproducto: ~0 rows (aproximadamente)
...
BEGIN
	...
	if(p_opcion=2)then
		...
		WHERE IdPermisoHerramienta = p_IdPermisoProducto;
	END if;
	if(p_opcion=3)then
